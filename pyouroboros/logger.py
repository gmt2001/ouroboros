        ...
        for item in self.filtered_strings:
            try:
                if item in record.msg:
                    record.msg = record.msg.replace(item, 8 * '*' + (item[-5:] if len(item) > 5 else ''))
                if any(item in str(arg) for arg in record.args):
                    record.args = tuple(arg.replace(item, 8 * '*' + (item[-5:] if len(item) > 5 else '')) if isinstance(arg, str) else arg
